---
friends: true
title: 友情链接
contentPosition: after
groups:
  - title: 导航站
    list:
      - name: Minecraft 导航
        link: https://mcjpg.org/nav/
        avatar: https://mcjpg.org/logo.png
        desc: MCJPG 社区 MC 导航
      - name: MC 导航网
        link: https://www.mcnav.net/
        avatar: https://www.mcnav.net/wp-content/uploads/2021/08/1627802438-logo.gif
        desc: Minecraft 网址导航站
      - name: 所见皆是Minecraft
        link: https://mcisee.top/
        avatar: https://mcisee.top/assets/icon/favicon.ico
        desc: 玩家可以从这里挖掘Minecraft常用资源
  - title: 资源站
    list:
      - name: CurseForge
        link: https://www.curseforge.com/minecraft
        avatar: /icon/CurseForge.svg
        desc: A world of endless gaming possibilities for modders and gamers alike.
      - name: Modrinth
        link: https://modrinth.com/
        avatar: https://toolb.cn/favicon/modrinth.com
        desc: The place for Minecraft
---